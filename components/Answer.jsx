...
// react component that takes two props
export const Answer = ({ answers, questionId }) => {

  // useState to keep track of the selected answer, fetched data, and loading status
  const [selected, setSeleceted] = useState(null)
  const [data, setData] = useState(null)
  const [loading, setLoading] = useState(false)

  // runs whenever the questionId or selected value changes.
  useEffect(() => {
    let subscribed = true
    if (selected) {
      setLoading(true)
